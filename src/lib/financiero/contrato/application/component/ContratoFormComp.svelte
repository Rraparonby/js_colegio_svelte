...
export let contrato_prop: Contrato = new Contrato()

const emit = createEventDispatcher()

//------------------ GENERAL --------------------
let title = 'Formulario Contrato';
...
export const handleAction_ClickTableRow = (contrato1:Contrato) => {
	//ocultarMensajeAlerta();			
	tipo_accion = Constantes.SELECCIONAR;			
	abrirModalFormGeneral();
	
	if(Constantes.IS_DEVELOPING == true) {
		text_id_aux = contrato1.id.toString();
	}
				
	contrato.id = contrato1.id;
	contrato.created_at = contrato1.created_at;
	
	contrato.updated_at = contrato1.updated_at;
	contrato.anio = contrato1.anio;
	contrato.valor = contrato1.valor;
	contrato.fecha = contrato1.fecha;
	
	contrato.firmado = Boolean(contrato1.firmado);	
};
...
export const nuevoDatos = async () => {
	...
	const data_json:ContratoReturnView = await nuevoDatosData(module,controller,contrato);
...
export const actualizarDatos = async () => {	
	...
	const data_json:ContratoReturnView = await actualizarDatosData(module,controller,contrato);
...
export const eliminarDatos = async () => {
	...
	const data_json:ContratoReturnView = await eliminarDatosData(module,controller,contrato);
...
const getFks = async () => {
	...
	const data:ContratoReturnView = await getFksData(module,controller);
		...
		<div class="modal_form_general_body">
			...
			<div id="div_contrato_form_general">
				...
				<form id="contrato_form_general" 
					...
					<label for="firmado" class="form-label">Firmado</label>
					<input  type="checkbox" 
