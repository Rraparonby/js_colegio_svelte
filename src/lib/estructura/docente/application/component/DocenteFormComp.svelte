...
export let docente_prop: Docente = new Docente()

const emit = createEventDispatcher()

//------------------ GENERAL --------------------
let title = 'Formulario Docente';
...
export const handleAction_ClickTableRow = (docente1:Docente) => {
	//ocultarMensajeAlerta();			
	tipo_accion = Constantes.SELECCIONAR;			
	abrirModalFormGeneral();
	
	if(Constantes.IS_DEVELOPING == true) {
		text_id_aux = docente1.id.toString();
	}
				
	docente.id = docente1.id;
	docente.created_at = docente1.created_at;
	
	docente.updated_at = docente1.updated_at;
	docente.nombre = docente1.nombre;
	docente.apellido = docente1.apellido;
	docente.fecha_nacimiento = docente1.fecha_nacimiento;
	
	docente.anios_experiencia = docente1.anios_experiencia;
	docente.nota_evaluacion = docente1.nota_evaluacion;
};
...
export const nuevoDatos = async () => {
	...
	const data_json:DocenteReturnView = await nuevoDatosData(module,controller,docente);
...
export const actualizarDatos = async () => {	
	...
	const data_json:DocenteReturnView = await actualizarDatosData(module,controller,docente);
...
export const eliminarDatos = async () => {
	...
	const data_json:DocenteReturnView = await eliminarDatosData(module,controller,docente);
		...
		<div class="modal_form_general_body">
			...
			<div id="div_docente_form_general">
				...
				<form id="docente_form_general" 
					...
					<label for="anios_experiencia" class="form-label">Anios Experiencia</label>
					<input  type="text" 
							id="anios_experiencia" name="anios_experiencia"
							class="form-control" placeholder="Anios Experiencia"
							bind:value={docente.anios_experiencia} />
					
					<label for="nota_evaluacion" class="form-label">Nota Evaluacion</label>
					<input  type="text" 
							id="nota_evaluacion" name="nota_evaluacion"
							class="form-control" placeholder="Nota Evaluacion"
